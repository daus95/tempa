...
PROMPT_DIR, ACTIVE_WORKSPACE_POINTER) into a per-test tmp_path, so no test ever reads or
writes the real dev machine's actual install folder or its real `.active-workspace`
pointer (which, outside tests, may point at a real, unrelated workspace)."""
