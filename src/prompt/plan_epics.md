...
- EPIC DEPENDENCIES MUST BE ACYCLIC, and the array order MUST be a valid execution order. Epics are implemented strictly in the order you write them into config.json, one after another — so every epic an epic depends on has to come BEFORE it. Never write a dependency that points forward, and never write two epics that depend on each other. A phrase like "bidirectional with EPIC-02" or "EPIC-04 later replaces EPIC-02's temporary implementation" is a CYCLE, not a dependency note: it cannot be executed in any order, and it deadlocks the runner — each epic waits for the other, and neither can finish. Seen live: an epic whose last feature required rewriting a second epic's code, while that second epic was itself recorded as blocked on the first; both sat unfinished and the run halted. If two bodies of work genuinely need each other, that is one of three things, and you must pick one and write it that way: (a) ONE epic containing both, (b) two epics where the earlier one ships a real, permanent implementation the later one only CONSUMES, or (c) the later epic's replacement work moved INTO the earlier epic as its own feature. Writing the temporary-then-replaced version and hoping the order works out is not an option.
