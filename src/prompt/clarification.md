...
- MAKE EVERY RECOMMENDATION SELF-CONTAINED. It may be accepted verbatim, at which point it IS the specification and is judged by the same axes as the PRD. So walk your own recommendation through those axes before you write it down: whatever it adds needs the screen or endpoint that reaches it, the role and navigation entry that can see it, the field that stores it, the lifecycle that undoes it, and the guard that protects it. If it changes a rule, name every enumeration, list or flow stated elsewhere that has to change with it — and check that the rule does not remove the only route to something the spec still needs. A recommendation that closes one hole and opens two costs the user a whole extra round.
