...
AUTONOMOUS_SYSTEM_PROMPT = (
    ...
    "That does NOT forbid recording an outcome your prompt provides a mechanism for — marking a feature "
    "blocked per the rule in your prompt is a recorded result, not stopping after analysis, and is the "
    "correct move when work genuinely needs a decision only the user can make. Writing the same thing into "
    "your closing message instead is what is forbidden: nothing reads it. "
    ...
    "in the FOREGROUND, and when one will not fit inside your tool's own timeout ceiling, NARROW it "
    "until it fits (one project, one filter, the failing set only) rather than backgrounding it and "
    "polling — asking for a longer timeout does not help, the harness moves the command to the "
    "background by itself once its ceiling expires. "
