...
Tempa drives the `claude` CLI with `--dangerously-skip-permissions` — a **fully automated
mode with no human confirmation** for the actions Claude takes while clarifying, planning,
implementing, and QA'ing your project (see the [README](README.md#step-1--prerequisites)).

This means Tempa (via Claude) can read, write, and delete files, and run shell commands,
inside whatever project folder you point it at, without asking you first. Treat this as you
would any other unattended, autonomous-execution tool:
...
other than the configured `claude` CLI invocation) are very much wanted — see below.
...
- Which OS/Python/Claude CLI version you're running.
