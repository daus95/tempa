...
| `tempa_process_group.py` | Session-scoped process containment: each backend CLI is spawned inside a container the OS tears down along with the session (a Job Object on Windows, a process group on POSIX), so a dev server / build daemon / watcher the agent started and never stopped dies with the session instead of being orphaned. It never asks what a process is, only who started it, which is what makes it work for any toolchain. Off via `terminate_leftover_processes`, in which case it is a no-op that restores the previous spawn exactly. Also owns the SIGINT/SIGTERM/`atexit` handlers `tempa_cli.run()` installs. |
...
| `tempa_git.py` | The one place Tempa runs git, and the ignore rules those commits depend on: `commit_workspace_changes` (`add -A` + `commit` in `workspace.root`), behind the `commit_after_qa_pass` and `finalize_checkpoint_commit` settings, and `ensure_prd_tracked`, which writes the `.gitignore` rules keeping `.tempa/` out of the workspace's repo while letting `.tempa/specs/prd/` into it. Both report an outcome and never raise — an auto-commit must never be able to fail a run. |
...
| `dashboard_api_clarify.py` | Clarification pane: render one findings file, render a single earlier finding read-only for the reference drawer (`read_finding`), write answers back into it (`apply_answers_to_file`), the skip-minor toggle, and the server-side gate on starting a Finalized Clarification run. |
...
| `dashboard_clarify_render.py` | Turns parsed findings into the HTML shown in the Clarification pane (a small hand-rolled markdown renderer, not a dependency). Takes an optional `linkify` hook — defaulting to identity, so output is unchanged without one — applied to the title/where/question/recommendation of every finding. Takes the "decided elsewhere" note the same way: an optional `overlaps` map, absent by default, rendered between a finding's recommendation and its answer controls. |
| `dashboard_clarify_overlap.py` | Which EARLIER findings already name a surface — a `backtick` field, a **bold** entity, a "quoted" message — that the finding being answered names too, so the Clarification pane can say so before "Follow the recommendation" turns that recommendation into the PRD's text. Deterministic and text-only (no model, no PRD read); advisory, gating nothing. Keys named by a large fraction of the whole folder are dropped as vocabulary rather than reported on every finding. Reports every earlier round, applied or not, and labels which — an unapplied decision contradicts something no document shows yet, an applied one is already in the PRD the round was evaluated against. |
| `dashboard_spec_refs.py` | Resolves the spec references a finding cites (`M07-FR-03`, `§13.7`, a file path) to a PRD file and line, and rewrites them into `a.spec-ref` links in the rendered HTML. Indexes the PRD at render time — so links work on rounds already on disk and line numbers can't go stale — and records only ids the PRD *defines* (heading / table id column / bold lead-in), which is what keeps a finding's references to earlier findings (`R8 M2`) out without naming any project's id vocabulary. Fails open: an unreadable PRD yields an empty index and linkification becomes a no-op. |
